...
// static int frag_pages = 100000;
// module_param(frag_pages, int, 0644);
// MODULE_PARM_DESC(frag_pages, "Number of small pages to allocate for fragmentation");

// static int test_order = 10; // 2^order pages
// module_param(test_order, int, 0644);
// MODULE_PARM_DESC(test_order, "Order for large allocation (size = 2^order * PAGE_SIZE)");

// static int free_stride = 2;
// module_param(free_stride, int, 0644);
// MODULE_PARM_DESC(free_stride, "Free every Nth page to create fragmentation");

// static int repeat = 10;
// module_param(repeat, int, 0644);
// MODULE_PARM_DESC(repeat, "Number of times to repeat the allocation test");
...
//     pr_info("kmalloc vs vmalloc timed demo starting\n");

//     pages = kmalloc_array(frag_pages, sizeof(void *), GFP_KERNEL);
//     if (!pages)
//         return -ENOMEM;

//     /* Step 1: Fragment memory */
//     for (i = 0; i < frag_pages; i++) {
//         pages[i] = kmalloc(PAGE_SIZE, GFP_KERNEL);
//         if (!pages[i])
//             break;
//     }
//     allocated = i;

//     pr_info("Allocated %d/%d small pages\n", allocated, frag_pages);

//     /* Step 2: Free some pages to create holes */
//     for (j = 0; j < allocated; j += free_stride) {
//         kfree(pages[j]);
//         pages[j] = NULL;
//     }

//     pr_info("Freed every %d-th page to fragment memory\n", free_stride);

//     size = (1 << test_order) * PAGE_SIZE;
//     pr_info("Test allocation size: %zu bytes (%d pages)\n",
//             size, 1 << test_order);
...
//         if (!kptr) {
//             pr_info("[run %d] kmalloc FAILED in %llu ns\n",
//                     i, t2 - t1);
//         } else {
//             pr_info("[run %d] kmalloc SUCCEEDED in %llu ns\n",
//                     i, t2 - t1);
//             kfree(kptr);
//         }
...
//         if (!vptr) {
//             pr_err("[run %d] vmalloc FAILED in %llu ns\n",
//                    i, t2 - t1);
//         } else {
//             pr_info("[run %d] vmalloc SUCCEEDED in %llu ns\n",
//                     i, t2 - t1);
//             vfree(vptr);
//         }
//     }
...
//     pr_info("kmalloc vs vmalloc timed demo exiting\n");
// }
...
// MODULE_LICENSE("GPL");
// MODULE_AUTHOR("ECAM");
// MODULE_DESCRIPTION("kmalloc vs vmalloc fragmentation + latency demo");